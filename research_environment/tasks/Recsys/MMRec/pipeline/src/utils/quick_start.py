...
def expand_hyper_grid(config):
    hyper_parameters = list(config['hyper_parameters'] or [])
    if "seed" not in hyper_parameters:
        hyper_parameters = ['seed'] + hyper_parameters
    hyper_ls = []
    for name in hyper_parameters:
        hyper_ls.append(_ensure_list(config[name]))
    combinators = list(product(*hyper_ls))
    return hyper_parameters, combinators
...
def quick_start(model, dataset, config_dict, save_model=True, mg=False):
    ...
    # hyper-parameters
    hyper_parameters, combinators = expand_hyper_grid(config)
    config['hyper_parameters'] = hyper_parameters
    total_loops = len(combinators)
    for hyper_tuple in combinators:
        # random seed reset
        for j, k in zip(hyper_parameters, hyper_tuple):
            config[j] = k
        init_seed(config['seed'])

        logger.info('========={}/{}: Parameters:{}={}======='.format(
            idx+1, total_loops, hyper_parameters, hyper_tuple))
    ...
    for (p, k, v) in hyper_ret:
        logger.info('Parameters: {}={},\n best valid: {},\n best test: {}'.format(hyper_parameters,
