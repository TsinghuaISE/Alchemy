...
class Config(object):
    ...
    def __init__(self, model=None, dataset=None, config_dict=None, mg=False):
        ...
        # track whether gpu id is explicitly provided to keep priority over env vars
        self._gpu_id_override = 'gpu_id' in config_dict
        # model type
    ...
    def _load_dataset_model_config(self, config_dict, mg):
        ...
        if model_config_path:
            if not os.path.isabs(model_config_path):
                model_config_path = os.path.abspath(os.path.join(os.getcwd(), model_config_path))
            file_list.append(model_config_path)
        else:
    ...
    def _init_device(self):
        ...
        cfg_gpu = self.final_config_dict.get('gpu_id')

        if use_gpu:
            if self._gpu_id_override:
                os.environ["CUDA_VISIBLE_DEVICES"] = str(cfg_gpu)
                env_gpu = os.environ.get("CUDA_VISIBLE_DEVICES")
            elif env_gpu:
        ...
        if use_gpu and env_gpu:
            # reflect effective GPU setting in config for logging
            self.final_config_dict['gpu_id'] = env_gpu
