...
class Model(nn.Module):
    ...
    def anomaly_detection(self, x_enc):
        """
        Anomaly detection method using encoder-decoder reconstruction.
        Args:
            x_enc: [B, L, D]
        Returns:
            dec_out: [B, L, D]
        ...
        # Use the latter half of encoder input for recent information
        mean = torch.mean(x_enc, dim=1, keepdim=True)  # [B, 1, D]
        
        # Decompose the latter half
        seasonal_init, trend_init = self.decomp(x_enc)  # [B, L, D]
        
        # For anomaly detection, we reconstruct the full sequence
        # Decoder input: seasonal component with zeros as placeholders
        dec_inp_seasonal = seasonal_init  # [B, L, D]
        dec_inp_trend = trend_init  # [B, L, D]
        
        # Decoder embedding
        dec_out = self.dec_embedding(dec_inp_seasonal, None)  # [B, L, d_model]
        
        # Decoder forward
        seasonal_part, trend_part = self.decoder(dec_out, enc_out, x_mask=None, cross_mask=None,
                                                 trend=dec_inp_trend)  # [B, L, c_out], [B, L, c_out]
        
        # Final prediction: seasonal + trend
        dec_out = seasonal_part + trend_part  # [B, L, c_out]
