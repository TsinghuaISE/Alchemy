...
class Model(nn.Module):
    ...
    def classification(self, x_enc, x_mark_enc):
        """
        x_enc: [B, L, enc_in]
        x_mark_enc: [B, L] padding mask
        Returns: [B, num_class]
        """
        # Instance normalization
        x_enc = x_enc - x_enc.mean(dim=1, keepdim=True).detach()
        x_enc = x_enc / (x_enc.std(dim=1, keepdim=True).detach() + 1e-5)
        
        # Embedding
        enc_out = self.enc_embedding(x_enc)  # [B, L, d_model]
        
        # Encoder (extract growth and seasonal components)
        enc_out, B_list, S_list = self.encoder(enc_out)
        
        # Use final encoder output for classification
