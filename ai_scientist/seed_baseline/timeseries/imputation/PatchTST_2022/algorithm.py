...
class Model(nn.Module):
    ...
    def imputation(self, x_enc, x_mark_enc=None, x_dec=None, x_mark_dec=None, mask=None):
        """
        Imputation method for PatchTST
        
        Args:
            x_enc: [B, L, C] - Input sequence with missing values
            mask: [B, L, C] - Mask indicating observed (1) and missing (0) values
        ...
        if mask is not None:
            ...
            sum_vals = (x_enc * mask_expanded).sum(dim=1, keepdim=True)
            count_vals = mask_expanded.sum(dim=1, keepdim=True).clamp(min=1)
            means = sum_vals / count_vals
            
            # Compute variance on observed values
            diff = (x_enc - means) * mask_expanded
            var = (diff ** 2).sum(dim=1, keepdim=True) / count_vals
        ...
        # Detach to prevent gradient flow through normalization statistics
        means = means.detach()
        stdev = stdev.detach()
        ...
        # Patch embedding: [B, L, C] -> [B*C, num_patch, d_model]
        enc_out, n_vars = self.patch_embedding(x_enc)
        
        # Transformer encoding
        enc_out = self.encoder(enc_out)  # [B*C, num_patch, d_model]
        
        # Projection head: [B*C, num_patch, d_model] -> [B, L, C]
