...
class Model(nn.Module):
    ...
    def imputation(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask):
        ...
        """
        # Normalization: normalize each variate independently
        # Calculate mean and std only on observed values (mask==1)
        if mask is not None:
            # Mask-aware normalization
            mask_sum = mask.sum(dim=1, keepdim=True) + 1e-5  # [B, 1, N]
            means = (x_enc * mask).sum(dim=1, keepdim=True) / mask_sum  # [B, 1, N]
            
            # Calculate variance on observed values
            x_centered = (x_enc - means) * mask
            stdev = torch.sqrt((x_centered ** 2).sum(dim=1, keepdim=True) / mask_sum + 1e-5)  # [B, 1, N]
            
            # Normalize
            x_enc = x_centered / stdev
            # Keep missing values as 0 after normalization
            x_enc = x_enc * mask
        else:
            # Standard instance normalization per variate
            means = x_enc.mean(1, keepdim=True).detach()  # [B, 1, N]
            x_enc = x_enc - means
            stdev = torch.sqrt(torch.var(x_enc, dim=1, keepdim=True, unbiased=False) + 1e-5).detach()
            x_enc = x_enc / stdev
        
        # Embedding: [B, L, N] -> [B, N, D]
        # Each variate (time series) becomes a token
        enc_out = self.embedding(x_enc)
        
        # Encoder: self-attention operates on variate tokens
        # [B, N, D] -> [B, N, D]
        enc_out, attns = self.encoder(enc_out, attn_mask=None)
        
        # Projection: project back to time series
        # [B, N, D] -> [B, N, L]
        dec_out = self.projection(enc_out)
        
        # Transpose back to [B, L, N]
