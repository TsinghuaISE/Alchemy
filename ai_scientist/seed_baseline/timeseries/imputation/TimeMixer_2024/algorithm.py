...
class Model(nn.Module):
    ...
    def imputation(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask):
        ...
        Args:
            x_enc: [B, seq_len, enc_in] - input sequence with missing values
            x_mark_enc: [B, seq_len, mark_dim] - temporal encoding
            x_dec: placeholder (not used in imputation)
            x_mark_dec: placeholder (not used in imputation)
            mask: [B, seq_len, enc_in] - mask where 1=observed, 0=missing
        Returns:
            imputed_data: [B, seq_len, c_out] - completed sequence
        """
        ...
        # Multi-scale decomposition and mixing
        # Generate multi-scale representations by downsampling
        enc_out_list = [enc_out]
        for i in range(self.down_sampling_layers):
            # Downsample by averaging with window
            down_pool = nn.AvgPool1d(
                kernel_size=self.down_sampling_window,
                stride=self.down_sampling_window
            )
            enc_out_down = down_pool(enc_out_list[-1].permute(0, 2, 1)).permute(0, 2, 1)
            enc_out_list.append(enc_out_down)

        # Apply PDM blocks
        for block in self.pdm_blocks:
            enc_out_list = block(enc_out_list)

        # Use the finest scale output
        dec_out = enc_out_list[0]  # [B, seq_len, d_model]
